...
\begin{itemize}
    ...
    \item \textbf{Biometrics}: Total Cholesterol, Blood Glucose levels, Serum Creatinine.
...
\begin{enumerate}
    \item \textbf{Column Selection}: Non-clinical columns (\texttt{patient\_id}, \texttt{diagnosis}, \texttt{readmission\_30d}, \texttt{mortality}) were dropped. Relevant clinical features including Creatinine were retained.
    \item \textbf{Missing Values}: Rows with missing values were removed using \texttt{dropna()} to ensure data integrity.
    ...
    \item \textbf{Train/Test Split}: Data was split 80/20 with \texttt{random\_state=42} for reproducible evaluation.
\end{enumerate}

\subsection{Ground Truth Logic}
A risk scoring formula was adapted from the \textbf{Framingham Risk Score} framework \cite{framingham}, a widely-used clinical tool for cardiovascular risk assessment. Our adapted formula calculates a weighted sum of normalized risk factors (Age, BMI, Blood Pressure, Cholesterol, Glucose, Creatinine, Diabetes, Hypertension), scaled to a 0--100 range. The weights reflect the relative clinical importance of each factor in cardiovascular and metabolic risk prediction.
...
The Logistic Regression model (with \texttt{class\_weight='balanced'} to handle class imbalance) achieved an overall accuracy of \textbf{97.45\%}.
...
\midrule
Low & 0.97 & 1.00 & 0.98 & 496 \\
Medium & 1.00 & 0.96 & 0.98 & 1408 \\
High & 0.73 & 1.00 & 0.85 & 96 \\
\bottomrule
\end{tabular}
...
\section{Conclusion}
This project successfully demonstrates the application of machine learning in clinical risk assessment. The \textbf{MediRisk AI} system provides a robust, accurate, and user-friendly tool for health monitoring. With an \textbf{$R^2$ score of 1.00} and \textbf{97.45\% classification accuracy}, the system proves highly effective within the scope of the synthetic dataset. The risk scoring formula, adapted from the Framingham Risk Score framework, provides clinically grounded risk estimates. Future work could involve integrating real-world clinical data and expanding the feature set to include genetic markers and lifestyle factors.
...
\bibitem{framingham}
D'Agostino, R.B., et al. (2008). General Cardiovascular Risk Profile for Use in Primary Care. \textit{Circulation}, 117(6), 743-753.
\end{thebibliography}
