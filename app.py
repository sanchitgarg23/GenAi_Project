...
def preprocess_data(df):
    ...
    # NOTE: We keep 'mortality' as our ground-truth outcome label!
...
def train_models(df):
    ...
    # ----- 1. CLASSIFICATION SETUP (Predicting Mortality) -----
    y_clf = df['mortality']
    ...
    # ----- 2. REGRESSION SETUP (Predicting Systolic BP) -----
    y_reg = df['Systolic_BP']
    features_reg = ['Age', 'BMI', 'Diastolic_BP', 'Cholesterol_Total', 'Glucose', 'Creatinine', 'Diabetes', 'Hypertension', 'Gender_Encoded']
    ...
    y_dt_pred = dt_clf.predict(X_test_scaled_clf)
    y_reg_pred = lin_reg.predict(X_test_scaled_reg)
    ...
    metrics = {
        ...
        'dt_accuracy': accuracy_score(y_test_clf, y_dt_pred),
        'conf_matrix': confusion_matrix(y_test_clf, y_clf_pred),
        'class_report': classification_report(y_test_clf, y_clf_pred, target_names=['Survived', 'Deceased'], output_dict=True),
        'feature_names_clf': features_clf,
        'coefficients': log_reg.coef_[0]
    }

    return lin_reg, log_reg, dt_clf, scaler_clf, scaler_reg, le_gender, features_clf, features_reg, metrics
...
if raw_df is None:
    ...
    lin_reg, log_reg, dt_clf, scaler_clf, scaler_reg, le_gender, features_clf, features_reg, metrics = train_models(df)
    ...
    if submitted:
        ...
        input_data_clf = input_data_clf[features_clf]
        input_scaled_clf = scaler_clf.transform(input_data_clf)
        
        # Calculate Risk Score from pure Logistic Regression Probability.
        # Now that the dataset is medically correlated, the ML model correctly calculates probability natively!
        ...
        st.write(f"**Predicted Mortality Risk:** {risk_score:.1f}%")
    ...
    with col_reg:
        st.markdown("##### Linear Regression (Predicting Systolic BP)")
    ...
    with col_clf:
        st.markdown("##### Logistic Regression (Mortality)")
        st.write(f"**Accuracy:** {metrics['accuracy']*100:.2f}%")
        st.markdown("##### Decision Tree (Mortality)")
        st.write(f"**Accuracy:** {metrics['dt_accuracy']*100:.2f}%")
        
        report_df = pd.DataFrame(metrics['class_report']).transpose()
        report_df = report_df.loc[['Survived', 'Deceased'], ['precision', 'recall', 'f1-score', 'support']]
    ...
    with viz_col1:
        st.markdown("##### Confusion Matrix (Mortality)")
    ...
    with viz_col2:
        st.markdown("##### Feature Importance (Logistic Reg)")
